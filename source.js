(function () {
  ...
  function layCacheRedirect(khoaCache) {
    ghiLog('Đang kết nối với cơ sở dữ liệu vanhcheat.rf.gd...', 'system');
    GM_xmlhttpRequest({
      method: 'GET',
      ...
      onerror: function () {
        ghiLog('Lỗi kết nối vanhcheat.rf.gd. Kích hoạt nhận diện AI...', 'error');
        batDauPipelineOCR();
      },
    });
  }
  ...
  function dongBoLenServer(khoaDongBo, tenMienDongBo) {
    ghiLog('Đang đồng bộ hóa dữ liệu lên vanhcheat.rf.gd...', 'system');
    GM_xmlhttpRequest({
      method: 'POST',
      url: SAVE_URL,
      ...
      onerror: function () {
        ghiLog('Không gọi được api.php trên vanhcheat.rf.gd.', 'error');
      },
    });
  }
